...
import { useCubeStore2x2 } from '../../store/cubeStore2x2';
...
// 2x2 specific gesture matrix
const GESTURE_MATRIX_2x2 = {
  F: [
...
// Convert 3D cubie position to 2D face matrix position for 2x2
function getCubieMatrixPosition2x2(cubiePos: Vector3, face: Face): [number, number] | null {
  let x: number, y: number;
  
  switch (face) {
    case 'F': // Front face (z = 0.5)
      if (Math.abs(cubiePos.z - 0.5) > 1e-6) return null;
      x = cubiePos.x;
      y = cubiePos.y;
      break;
    case 'B': // Back face (z = -0.5)
      if (Math.abs(cubiePos.z + 0.5) > 1e-6) return null;
      x = -cubiePos.x; // Mirror x-axis to maintain consistent orientation
      y = cubiePos.y;
      break;
    case 'L': // Left face (x = -0.5)
      if (Math.abs(cubiePos.x + 0.5) > 1e-6) return null;
      x = cubiePos.z;
      y = cubiePos.y;
      break;
    case 'R': // Right face (x = 0.5)
      if (Math.abs(cubiePos.x - 0.5) > 1e-6) return null;
      x = -cubiePos.z; // Mirror z to maintain consistent orientation
      y = cubiePos.y;
      break;
    case 'U': // Top face (y = 0.5)
      if (Math.abs(cubiePos.y - 0.5) > 1e-6) return null;
      x = cubiePos.x;
      y = -cubiePos.z;
      break;
    case 'D': // Bottom face (y = -0.5)
      if (Math.abs(cubiePos.y + 0.5) > 1e-6) return null;
      x = cubiePos.x;
      y = cubiePos.z;
      break;
...
// Get rotation action based on cubie position and drag direction for 2x2
function getRotationFromGesture2x2(
  frontFace: Face,
  cubiePos: Vector3,
  dragDirection: 'up' | 'down' | 'left' | 'right'
): {face: Face; clockwise: boolean} | null {
  const matrixPos = getCubieMatrixPosition2x2(cubiePos, frontFace);
  if (!matrixPos) return null;
  
  const [x, y] = matrixPos;
  const faceMatrix = GESTURE_MATRIX_2x2[frontFace];
...
function CubeGroup2x2() {
  ...
  // Function to determine which cubies belong to a face for 2x2
  const getCubiesForFace2x2 = (face: Face): string[] => {
    ...
    validCubies.forEach(cubie => {
      ...
      switch (face) {
        case 'F': belongsToFace = Math.abs(z - 0.5) < 1e-6; break;
        case 'B': belongsToFace = Math.abs(z + 0.5) < 1e-6; break;
        case 'L': belongsToFace = Math.abs(x + 0.5) < 1e-6; break;
        case 'R': belongsToFace = Math.abs(x - 0.5) < 1e-6; break;
        case 'U': belongsToFace = Math.abs(y - 0.5) < 1e-6; break;
        case 'D': belongsToFace = Math.abs(y + 0.5) < 1e-6; break;
      }
  ...
  useFrame((state) => {
    ...
    if (animationStartTimeRef.current === 0) {
      ...
      const faceCubies = getCubiesForFace2x2(animatingFace);
...
function FaceDetector2x2({ controlsRef }: { controlsRef: React.RefObject<any> }) {
  ...
  const getCubieFaces2x2 = (position: Vector3): Face[] => {
    const faces: Face[] = [];
    const threshold = 0.1;
    
    if (Math.abs(position.x - 0.5) < threshold) faces.push('R');
    if (Math.abs(position.x + 0.5) < threshold) faces.push('L');
    if (Math.abs(position.y - 0.5) < threshold) faces.push('U');
    if (Math.abs(position.y + 0.5) < threshold) faces.push('D');
    if (Math.abs(position.z - 0.5) < threshold) faces.push('F');
    if (Math.abs(position.z + 0.5) < threshold) faces.push('B');
  ...
  const handlePointerDown = (event: PointerEvent) => {
    if (useCubeStore2x2.getState().isAnimating) return;
    ...
    const cubiePos = new Vector3(
      Math.round(cubieWorldPos.x * 2) / 2, // Round to nearest 0.5
      Math.round(cubieWorldPos.y * 2) / 2,
      Math.round(cubieWorldPos.z * 2) / 2
    );

    const frontFace = getFrontFacingFace(camera);
    const cubieData = {
      position: cubiePos,
      faces: getCubieFaces2x2(cubiePos),
      clickedFace: getFaceFromNormal(hit.face!.normal.clone().transformDirection(hit.object.matrixWorld)),
      frontFace: frontFace,
      matrixPosition: getCubieMatrixPosition2x2(cubiePos, frontFace)
    };
  ...
  const handlePointerMove = (event: PointerEvent) => {
    if (!dragInfo.current || useCubeStore2x2.getState().isAnimating) return;
  ...
  const handlePointerUp = (event: PointerEvent) => {
    if (dragInfo.current && isDragging.current && !useCubeStore2x2.getState().isAnimating) {
      const dx = event.clientX - dragInfo.current.startPos.x;
      const dy = event.clientY - dragInfo.current.startPos.y;
      ...
      const rotationAction = getRotationFromGesture2x2(frontFace, cubiePos, dragDirection);
      
      if (rotationAction) {
        const { face: targetFace, clockwise } = rotationAction;
        console.log(`2x2: Rotating face ${targetFace} ${clockwise ? 'clockwise' : 'counter-clockwise'}`);
        
        useCubeStore2x2.getState().setRotationDirection(clockwise);
        useCubeStore2x2.getState().rotateFace(targetFace, clockwise);
      }
    }
...
function Scene2x2() {
  ...
  return (
    <>
      ...
      <CubeGroup2x2 />
      <FaceDetector2x2 controlsRef={controlsRef} />
      <OrbitControls 
        ref={controlsRef}
        enablePan={false} 
        enableZoom={true}
